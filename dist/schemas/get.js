...
module.exports = base.keys({
   ...
   */
  state: Joi.string().optional().valid(['unread', 'archive', 'all']),

  /**
   * `0`: only return un-favorited items.
   * `1`: only return favorited items.
   */
  favorite: Joi.number().integer().optional().min(0).max(1),
   ...
   */
  contentType: Joi.string().optional().valid(['article', 'image', 'video']),
   ...
   */
  sort: Joi.string().optional().valid(['newest', 'oldest', 'title', 'site']),
   ...
   */
  detailType: Joi.string().optional().valid(['simple', 'complete']),

  search: Joi.string().optional().description('Only return items whose title or url contain the search string.'),

  domain: Joi.string().optional().description('Only return items from a particular domain.'),

  since: Joi.date().optional().description('Only return items modified since the given since unix timestamp.'),

  count: Joi.number().integer().optional().min(0).description('Only return count number of items.'),

  offset: Joi.number().integer().optional().min(0).description('Used only with count; start returning from offset position of results.')
});